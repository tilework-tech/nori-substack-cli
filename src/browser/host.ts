...
export function resolveVncStack(xvfbExists: boolean, home: string, vncRoot = path.join(home, ".local/vnc-root")): VncStackPlan {
  if (xvfbExists) return { useProot: false, vncBin: "/usr/bin", novncWeb: "/usr/share/novnc", websockify: ["websockify"], vncRoot };
...
export function buildNovncUrl(sessionBaseUrl: string, novncPort: number): string {
...
function readState(): HostState | undefined {
  const target = statePath();
  if (!existsSync(target)) return undefined;
  return JSON.parse(readFileSync(target, "utf8")) as HostState;
}
...
async function waitFor(predicate: () => boolean | Promise<boolean>, timeoutMs: number, intervalMs: number): Promise<boolean> {
...
function findMissing(plan: VncStackPlan): string[] {
  const missing: string[] = [];
  if (plan.useProot) {
    for (const relative of ["usr/bin/proot", "usr/bin/Xtigervnc", "usr/bin/xkbcomp", "usr/bin/fluxbox"]) {
      if (!existsSync(path.join(plan.vncRoot, relative))) missing.push(path.join(plan.vncRoot, relative));
    }
    if (!onPath("python3")) missing.push("python3");
  } else {
    for (const binary of ["Xvfb", "x11vnc", "fluxbox", "websockify"]) if (!onPath(binary)) missing.push(binary);
  }
  if (!existsSync(plan.novncWeb)) missing.push(plan.novncWeb);
  return missing;
}

function stackEnvironment(plan: VncStackPlan, home: string): NodeJS.ProcessEnv {
  const environment: NodeJS.ProcessEnv = { ...process.env, DISPLAY: `:${DISPLAY_NUM}` };
  if (!plan.useProot) return environment;
...
function chromiumEnvironment(plan: VncStackPlan, home: string): NodeJS.ProcessEnv {
...
function spawnDetached(command: string, args: string[], environment: NodeJS.ProcessEnv, logFile: string): number {
...
function writeWatchdog(plan: VncStackPlan, logDir: string): string {
...
async function startBridge(plan: VncStackPlan, chromium: string, home: string): Promise<number[]> {
...
function stopBridge(): unknown {
  const state = readState();
  if (!state) return { stopped: false, reason: "no bridge running" };
  for (const pid of state.pids) {
    try { process.kill(-pid, "SIGTERM"); }
    catch { try { process.kill(pid, "SIGTERM"); } catch { /* process already gone */ } }
  }
...
export async function hostBrowser(options: HostOptions): Promise<unknown> {
  if (options.stop) return stopBridge();

  const running = readState();
  if (running && running.pids.some(isAlive)) return { ...running, status: "awaiting_human", reused: true, next: nextSteps };

  const sessionUrl = (options.sessionUrl ?? process.env.NORI_SESSION_URL ?? "").trim();
  if (!sessionUrl) throw new CliError("SESSION_URL_MISSING", "No --session-url given and NORI_SESSION_URL is unset; cannot build the human noVNC link.", 2, true);
