...
export async function exportPostArtifact(client: PublicClient, postUrl: string): Promise<unknown> {
  ...
  $(".mention-wrap,[data-component-name='MentionToDOM']").each((_index, element) => {
    ...
    try {
      const attributes = rawAttributes ? JSON.parse(rawAttributes) as Record<string, unknown> : undefined;
...
function renderQuotedBlock(node: NoteNode): string {
  ...
  if (isNodeType(node, "blockquote")) return (node.content ?? []).map(renderQuotedBlock).filter(Boolean).join("\n\n");
  return renderBlock(node);
}
function renderBlock(node: NoteNode): string {
  if (isNodeType(node, "blockquote")) return toBlockquote((node.content ?? []).map(renderQuotedBlock).filter(Boolean).join("\n\n"));
