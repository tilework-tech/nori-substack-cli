...
import { commandFamilies } from "./specs.js";
import { CliError, normalizeError } from "../core/errors.js";
import { writeError, writeResult } from "../core/output.js";
const ROOT_SOURCE = "src/index.ts";
function distance(a, b) { const row = Array.from({ length: b.length + 1 }, (_, i) => i); for (let i = 1; i <= a.length; i++) {
    let previous = row[0];
    row[0] = i;
...
function validateTopLevel(argv) {
    const input = argv[0];
    if (!input || input.startsWith("-") || commandFamilies.some((family) => family.name === input))
        return;
...
function addOption(command, spec) {
...
function addOperation(program, family, familyName, spec) {
    ...
    for (const option of spec.options ?? [])
        addOption(operation, option);
    operation.action(async (localOptions) => {
        try {
            const globals = program.opts();
            const result = await executeCommand(familyName, spec.name, localOptions, globals);
            writeResult(`${familyName}.${spec.name}`, result, (globals.format ?? "json"));
        }
        catch (error) {
            const normalized = normalizeError(error);
            writeError(normalized, (program.opts().format ?? "json"));
...
export function createProgram() {
    ...
    for (const familySpec of commandFamilies) {
        ...
        for (const operationSpec of familySpec.operations)
            addOperation(program, family, familySpec.name, operationSpec);
    }
    return program;
}
export async function run(argv = process.argv.slice(2)) {
    try {
        validateTopLevel(argv);
    }
    catch (error) {
