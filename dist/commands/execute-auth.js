...
function stringOption(options, names) { for (const name of names) {
    const value = options[name];
    if (typeof value === "string" && value.length > 0)
        return value;
} return undefined; }
function requiredString(options, names, label) { const value = stringOption(options, names); if (!value)
    throw new CliError("INVALID_ARGUMENT", `${label} is required.`, 2, false); return value; }
function globalStatePath(globals) { return typeof globals.state === "string" && globals.state ? globals.state : path.join(os.homedir(), ".local/share/noriagent/substack-storage-state.json"); }
...
async function executeImport(options, globals) {
...
async function executeExport(globals) {
...
async function executeStatus(globals) {
    const encoded = resolveStateEnvValue(globals.stateEnv);
    if (encoded) {
        const { state } = decodeStorageStateBase64(encoded);
        return { data: { authenticated: sessionCookies(state).length > 0, source: "environment", cookieCount: state.cookies.length } };
    }
    const source = globalStatePath(globals);
    if (!await fileExists(source))
        return { data: { authenticated: false, source: null, statePath: source, cookieCount: 0 } };
...
async function executeAuth(operation, options, globals) {
    if (operation === "import")
        return executeImport(options, globals);
    if (operation === "export")
        return executeExport(globals);
    if (operation === "status")
        return executeStatus(globals);
...
async function executePost(operation, options, globals) {
    ...
    if (operation === "publish") {
        ...
        if (options.dryRun === true)
            return describeDryRun({ method: "PUT", url, body, impact });
...
export async function executeAuthCommand(family, operation, options, globals) {
    if (family === "auth")
        return executeAuth(operation, options, globals);
    if (family === "profile" && operation === "me")
        return { data: await authenticatedRequest({ url: joinSubstackUrl(typeof globals.accountOrigin === "string" ? globals.accountOrigin : "https://substack.com", "/api/v1/user/profile/self"), globals }) };
    if (family === "post")
...
//# sourceMappingURL=execute-auth.js.map